...
const TokensDropdown = ({ tokens, selectedToken, emitToken }: { tokens: IToken[], selectedToken: IToken, emitToken: (token: IToken) => void; }) => {
  ...
  return (
    <div className="relative" ref={DropdownRef}>
      <button onClick={handleToggle} data-dropdown-toggle="dropdown" type="button" className={`flex justify-between items-center border-2 border-transparent bg-zinc-900 w-full rounded-lg text-sm px-4 py-2.5 placeholder-gray-600 focus:outline-none focus:border-pink-500 ${isOpen ? 'rounded-b-none border-b-0' : ""}`}>
